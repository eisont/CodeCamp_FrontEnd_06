  ...
  return (
    ...
    <S.Wrapper>
      {loggedIn?.fetchUserLoggedIn._id ? 
        <>
          <S.Point>
            <S.Text>{loggedIn?.fetchUserLoggedIn?.name} 님의 포인트 <u>{loggedIn?.fetchUserLoggedIn?.userPoint?.amount}</u>P</S.Text>
          </S.Point>
          <S.Menu>
      ...
      <S.MenuBasketBt>찜<S.BasketNum>{data?.fetchUseditemsCountIPicked}</S.BasketNum></S.MenuBasketBt>
      <S.MenuBasketBt>장바구니<S.BasketNum>{basketCount}</S.BasketNum></S.MenuBasketBt>
    </S.Wrapper>
    </>
  );
}
